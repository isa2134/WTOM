document.addEventListener('DOMContentLoaded', function () {
    ...
    function toggleFormVisibility(visible, isEdit = false) {
        ...
        if (formAcao)
            formAcao.value = isEdit ? 'editar' : 'cadastrar';
        if (!visible) {
            resetForm();
    }
    }
    ...
    function resetForm(dataEventoKey = getSelectedDateKey()) {
        ...
        if (dataEventoEl)
            dataEventoEl.value = dataEventoKey;
        if (idEventoEl)
            idEventoEl.value = '';
        if (formAcaoEl)
            formAcaoEl.value = 'cadastrar';
        if (fileNameDisplay)
            fileNameDisplay.textContent = '';
        if (anexoUrlInput)
            anexoUrlInput.disabled = false;
    ...
    function renderCalendar() {
        ...
        if (monthHeaderEl)
            monthHeaderEl.textContent = `${months[currentMonth].toUpperCase()} ${currentYear}`;
        if (yearDisplayEl)
    ...
    function renderEventList(dateKey, events) {
        if (selectedDateDisplayEl)
            selectedDateDisplayEl.textContent = `Eventos em ${dateKey.split('-').reverse().join('/')}`;
        if (!eventListEl)
            return;
...
<div class="event-actions" data-id="${evt.id}">
    ${!evt.isRepetition ? `
        <a href="#" class="edit-event-btn" data-id="${evt.id}">
            <i class="fa-solid fa-pen-to-square"></i>
        </a>
    ` : ''}
    <a href="${CONTEXT_PATH}/CronogramaController?acao=excluir&id=${evt.id}&dataAtual=${getSelectedDateKey()}"
    ...
    function renderWeeklyView() {
        ...
        weekDates.forEach(d => {
            ...
            allDayEvents.forEach(evt => {
                ...
                div.onclick = () => {
                    if (window.canManage && !evt.isRepetition) {
                        loadFormForEdit(allEvents.find(x => x.id == evt.id));
                    }
                };
