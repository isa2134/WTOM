...
<head>
    ...
    <style>
        ...
        .olimpiada-info h4 { margin: 0; font-size: 17px; font-weight: 700; color: var(--accent); }
        .olimpiada-info small { color: var(--muted); font-size: 14px; }
        .olimpiada-actions { display: flex; gap: 8px; }
        .btn {
            border: none;
            border-radius: 8px;
            padding: 8px 14px;
        ...
        .btn-ver { background-color: #e9f1f3; color: var(--accent); border: 1px solid #c9d6da; }
        .btn-editar { background-color: var(--accent); color: white; }
        .btn-excluir { background-color: #DC3545; color: white; }
        .btn-default { background-color: #0F4C5C; color: white; }
        .modal {
            display: none; 
            position: fixed;
            z-index: 9999;
        ...
        #modalExcluir .modal-content { border-top: 6px solid #c62828; }
        @media (max-width: 768px) {
            main { margin-left: 0; padding: 20px; }
            .olimpiada-card { flex-direction: column; align-items: flex-start; gap: 15px; }
            .olimpiada-actions { width: 100%; justify-content: space-between; }
        }
    </style>
</head>
<body>
    ...
    <main>
        ...
        <button id="btnToggleFiltro" class="btn btn-ver" style="margin-bottom: 20px;">Filtrar Resultados</button>
        <div id="filtroArea" style="display:none;">
            <form action="olimpiada" method="get" style="display:flex; gap:10px; flex-wrap:wrap; align-items:center;">
                ...
                <select name="ordenarPor">
                    ...
                    <option value="data" ${param.ordenarPor == 'data' ? 'selected' : ''}>Data</option>
                    <option value="peso" ${param.ordenarPor == 'peso' ? 'selected' : ''}>Peso</option>
                </select>
                <button class="btn btn-editar" type="submit">Aplicar</button>
            </form>
        </div>
        ...
        <div id="modalVer" class="modal" style="display: none;">
    ...
    <script>
        ...
        const areaFiltro = document.getElementById("filtroArea");
        btnFiltro.onclick = () => {
            const isHidden = areaFiltro.style.display === "none";
            areaFiltro.style.display = isHidden ? "block" : "none";
            btnFiltro.textContent = isHidden ? "Fechar Filtros" : "Filtrar Resultados";
        };
        ...
        function abrirModalExcluir(id, nome) {
            ...
            timerExcluir = setInterval(() => {
                ...
                } else { btn.textContent = `Excluir (${s}s)`; }
            }, 1000);
        ...
        function fecharModal(id) {
            ...
            if(id === 'modalExcluir') clearInterval(timerExcluir);
        }
