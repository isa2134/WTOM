...
<c:set var="nomeOlimpiada" value="${nomeOlimpiada}" />
    
<main>

  <h1>Inscrições - ${nomeOlimpiada}</h1>

  <div class="top-actions">
    <form action="olimpiada" method="post">
      <input type="hidden" name="acao" value="listarOlimpiadaAdminProf">
  ...
  <c:choose>
    <c:when test="${not empty inscricoes}">
      <div class="container-cards">
        ...
        <c:forEach var="i" items="${inscricoes}">

          <div class="card">
            <div class="info">
              <h4>${i.getNome()}</h4>
              <small><strong>CPF:</strong> ${i.getCpf()}</small>
            </div>
    ...
    <c:otherwise>
      <p style="color:#6B7A81;">Esta olímpiada ainda não possui inscrições.</p>
    </c:otherwise>
  </c:choose>
  ...
  <div class="modal-content">
    ...
    <div style="display:flex; justify-content: space-between; margin-top: 18px;">
      ...
      <form id="formCancelar" action="olimpiada" method="post" style="width:48%;">
        <input type="hidden" name="acao" value="cancelarInscricaoAdminProf">
...
<script>
  ...
  function abrirModalCancelar(idOlimpiada, nome, idUsuario) {

    document.getElementById("textoCancelar").textContent =
      "Deseja realmente cancelar a inscrição de (" + nome + ") de id (" + idUsuario +"), na olímpiada  ('${nomeOlimpiada}') ?";

    document.getElementById("inputIdOlimpiada").value = idOlimpiada;
